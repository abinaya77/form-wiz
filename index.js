...
// MONGO DB ROUTES
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASSWORD}@cluster0.3c5n1sh.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;
...
async function run() {
    try {
        ...
        app.post('/payment-info', verifyJWT, async (req, res) => {
            ...
            let classesId = paymentInfo.classId;
...
//formify codes
let forms = []; // Array to store created forms


// Endpoint to create a new form
app.post('/api/forms', (req, res) => {
  const { title, fields } = req.body;
  const newForm = { id: forms.length + 1, title, fields };
  forms.push(newForm);
  res.json(newForm);
});
...
// Endpoint to get HTML code for a specific form
app.get('/api/forms/:id/html', (req, res) => {
  ...
  form.fields.forEach((field, index) => {
    console.log("field and index are", field, index);
    switch (field.type) {
      case 'text':
        formHTML += `<label for="field${index}">${field.label}:</label>\n`;
        formHTML += `<input type="text" id="field${index}" name="${field.label}" />\n`;
        break;
      case 'email':
        formHTML += `<label for="field${index}">${field.label}:</label>\n`;
        formHTML += `<input type="email" id="field${index}" name="${field.label}" />\n`;
        break;
      case 'textarea':
        formHTML += `<label for="field${index}">${field.label}:</label>\n`;
        ...
        break;
      case 'checkbox':
        formHTML += `<input type="checkbox" id="field${index}" name="${field.label}" />\n`;
        formHTML += `<label for="field${index}">${field.label}</label>\n`;
        break;
      case 'radio':
        formHTML += `<input type="radio" id="field${index}" name="${field.label}" value="${field.options[0]}" />\n`;
        formHTML += `<label for="field${index}">${field.label}</label>\n`;
        break;
      case 'select':
        formHTML += `<label for="field${index}">${field.label}:</label>\n`;
        ...
        formHTML += `</select>\n`;
        break;
      // Add more cases for additional field types here
      default:
        // Handle unsupported field types
        formHTML += `<p>Unsupported field type: ${field.type}</p>\n`;
    }
  });
